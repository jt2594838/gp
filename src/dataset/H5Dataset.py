...
class H5Dataset(data.Dataset):
    def __init__(self, root, train=True):
        ...
        if train:
            choices = np.random.choice(data_size, int(0.9 * data_size), replace=False)
        else:
            choices = np.random.choice(data_size, int(0.1 * data_size), replace=False)
        choices.sort()
...
def get_anzhen_dataset(root, train):
    return H5Dataset(root, train)